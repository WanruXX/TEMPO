...
class Dataset_ETT_hour(Dataset):
    ...
    def stl_resolve(self, data_raw, data_name):
        ...
        if (
            os.path.isfile(trend_pk)
            and os.path.isfile(seasonal_pk)
            and os.path.isfile(resid_pk)
        ):
            ...
            for i, col in enumerate(cols):
                ...
                trend_stamp[:, i] = torch.tensor(
                    np.array(res.trend.values), dtype=torch.float32
                )
                seasonal_stamp[:, i] = torch.tensor(
                    np.array(res.seasonal.values), dtype=torch.float32
                )
                resid_stamp[:, i] = torch.tensor(
                    np.array(res.resid.values), dtype=torch.float32
                )
...
class Dataset_Pred(Dataset):
    ...
    def __read_data__(self):
        ...
        data_raw = pd.DataFrame.join(df_time, pd.DataFrame(data))  # [border1:border2]
        trend_stamp, seasonal_stamp, resid_stamp = self.stl_resolve(
            data_raw=data_raw, period=self.period
        )
    ...
    def __getitem__(self, index):
        ...
        return (
            ...
            seq_x_mark,
            seq_y_mark,
            seq_trend.reshape(-1, 1),
            seq_seasonal.reshape(-1, 1),
            seq_resid.reshape(-1, 1),
        )
...
class Dataset_Kline(Dataset):
    def __init__(
        ...
        if size == None:
            self.seq_len = 24 * 4 * 4
            self.label_len = 24 * 4  # 'former' model, 'TimesNet', 'LightTS'
            self.pred_len = 24 * 4
        else:
            self.seq_len = size[0]
            self.label_len = size[1]
    ...
    def __read_data__(self):
        self.scaler = StandardScaler()
        df_raw = pd.read_csv(os.path.join(self.root_path, self.data_path))
        df_raw = df_raw.tail(int(self.percent / 100 * len(df_raw)))
